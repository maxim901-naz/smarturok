...
def chat_start(request, user_id):
    ...
    # проверяем, есть ли урок между пользователями
    has_lesson = LessonBooking.objects.filter(student=request.user, teacher=other).exists() or \
                 LessonBooking.objects.filter(student=other, teacher=request.user).exists()

    if not has_lesson:
        return render(request, "chat/no_access.html", {"other_user": other})

    # упорядочиваем по id, чтобы не дублировать чаты
    student, teacher = (request.user, other) if request.user.id < other.id else (other, request.user)
